...
fn main() -> int
{
	...
	assert(out["id1_child1"].size() == 1);  // matches the first "test2" Word

	assert(out.contains("id1_child2"));
	assert(out["id1_child2"].size() == 1);
	assert(any_cast<string>(out["id1_child2"][0]->states["test3"]->value) == "yoo3");

	assert(out.contains("id1_child2_grandchild1"));
	assert(out["id1_child2_grandchild1"].size() == 2);  // matches both "test4" Words

	assert(out.contains("id1_child2_grandchild2"));
	assert(out["id1_child2_grandchild2"].size() == 1);
